...
We can think of /continuous/ math as the calculation of things smoothly changing, in continuous motion---which calls for measurements that can capture what is happening as accurately as needed. One typical use of continuous math is describing the behavior of an object acted upon by a force, and the subsequent paths and trajectories the object might take. But when we say continuous, we mean /smooth/ continuous---as in not broken up into chunks---more than we mean /constant/ continuous. Again, one hallmark of continuous math is its extreme measurement accuracy. Consider the exact calculations necessary to send satellites such as [[https://en.wikipedia.org/wiki/Voyager_program][Voyager 1 and 2]] on missions into the solar system. Voyager 2 was placed into perfect orbits with both Jupiter and Saturn in order to gain speed from a gravitational “slingshot effect.” The precision of these calculations is phenomenal, to say the least.
...
Just by looking at the smooth, continuous line in the graph above, we see that any increment of time along the horizontal axis, however small we might make it, will result in some new distance along the vertical axis. For example,
...
So if we keep making the two conditions closer and closer, the jog from the lower section to the higher will get smaller . . . until it collapses back to a smooth-looking $f(x) = 0$
...
To go from the world of continuous to discrete, sometimes all we need to do is simplify. Consider this “regular” map, accurate spatially-geographically, meaning the area and angles haven’t been distorted:
...
Yes, exactly. What do we put on the right side of the equation? But then what does the left side mean? So perhaps if we make a calculation of some sort on the right side, the left side will be one of the stations. For example

\begin{align*}
S(15) = Annadale
\end{align*}

Clearly, this is a discrete problem since there are only whole stops, e.g., there is no such thing as a $.7695$-stop. That means we need to feed our equation whole numbers. Since Annadale is the fifteenth stop, $S(15)$ should be Annadale. But again, we don’t seem to have a way of expressing this as a proper function.
